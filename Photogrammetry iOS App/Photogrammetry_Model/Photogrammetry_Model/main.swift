...
import ArgumentParser  // Available from Apple resource: https://github.com/apple/swift-argument-parser
import Foundation
import os
import RealityKit
import Metal
...
private let logger = Logger(subsystem: "com.apple.sample.photogrammetry",
                            category: "Photogrammetry3Dmodel")

/// Checks to make sure at least one GPU meets the minimum requirements for object reconstruction.
...
// Implements the main command structure, defines the command-line arguments, and specifies the main run loop.
struct Photogrammetry3Dmodel: ParsableCommand {
    ...
    func run() {
        ...
        // The compiler may deinitialize these objects since they may appear to be unused.
        withExtendedLifetime((session, waiter)) {
            ...
            // Enter the infinite loop dispatcher used to process asynchronous blocks on the main queue.
                RunLoop.main.run()
            } catch {
...
Photogrammetry3Dmodel.main(
    ["/Users/piyushmanghani/Desktop/Earpods/Feb242022",
                             "/Users/piyushmanghani/Desktop/Earpods_medium.usdz",
                             "-d","medium",
                             "-o","sequential",
                             "-f","normal"])
} else {
