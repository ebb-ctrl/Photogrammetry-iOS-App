...
/// This helper class loads the contents of an image capture folder. It uses asynchronous calls that run on a background queue and includes static methods for retrieving the top-level capture folder.
class CaptureFolderState: ObservableObject {
    ...
    /// This method populates the `CaptureInfo` array with all the image files contained in `captureDir`.
